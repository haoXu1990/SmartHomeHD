...
@interface NSString (SRWebSocket2)
...
@interface _SRRunLoopThread2 : NSThread
...
@implementation NSData (SRWebSocket2)
...
@implementation NSString (SRWebSocket2)
...
NSString *const SRWebSocketErrorDomain2 = @"SRWebSocketErrorDomain";
NSString *const SRHTTPResponseErrorKey2 = @"HTTPResponseStatusCode";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(SRWebSocket2 *webSocket,  NSData *data);

@interface SRIOConsumer2 : NSObject {
...
@interface SRIOConsumerPool2 : NSObject

- (id)initWithBufferCapacity:(NSUInteger)poolSize;

- (SRIOConsumer2 *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
- (void)returnConsumer:(SRIOConsumer2 *)consumer;

@end

@interface SRWebSocket2 ()  <NSStreamDelegate>
...
@implementation SRWebSocket2 {
    ...
    __strong SRWebSocket2 *_selfRetain;
    
    NSArray *_requestedProtocols;
    SRIOConsumerPool2 *_consumerPool;
}
...
- (void)_SR_commonInit;
{
    ...
    _consumerPool = [[SRIOConsumerPool2 alloc] init];
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if (responseCode >= 400) {
        ...
        [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain2 code:2132 userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"received bad response code from server %ld", (long)responseCode], SRHTTPResponseErrorKey2:@(responseCode)}]];
        return;
    }
    
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain2 code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"] forKey:NSLocalizedDescriptionKey]]];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain2 code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"] forKey:NSLocalizedDescriptionKey]]];
            return;
        }
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(SRWebSocket2 *self,  NSData *data) {
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(SRWebSocket2 *self, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(SRWebSocket2 *self, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [self _addConsumerWithDataLength:extra_bytes_needed callback:^(SRWebSocket2 *self, NSData *data) {
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        ...
        if (bytesWritten == -1) {
            [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain2 code:2145 userInfo:[NSDictionary dictionaryWithObject:@"Error writing to stream" forKey:NSLocalizedDescriptionKey]]];
             return;
        }
...
- (BOOL)_innerPumpScanner {
    ...
    SRIOConsumer2 *consumer = [_consumers objectAtIndex:0];
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode;
{
    if (_secure && !_pinnedCertFound && (eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable)) {
        ...
        if (sslCerts) {
            ...
            if (!_pinnedCertFound) {
                dispatch_async(_workQueue, ^{
                    [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain2 code:23556 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid server cert"] forKey:NSLocalizedDescriptionKey]]];
                });
                return;
            }
        }
...
@implementation SRIOConsumerPool2 {
...
- (SRIOConsumer2 *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
{
    SRIOConsumer2 *consumer = nil;
    if (_bufferedConsumers.count) {
        consumer = [_bufferedConsumers lastObject];
        [_bufferedConsumers removeLastObject];
    } else {
        consumer = [[SRIOConsumer2 alloc] init];
    }
...
@implementation NSURL (SRWebSocket2)
...
static _SRRunLoopThread2 *networkThread = nil;
static NSRunLoop *networkRunLoop = nil;

@implementation NSRunLoop (SRWebSocket2)

+ (NSRunLoop *)SR_networkRunLoop {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        networkThread = [[_SRRunLoopThread2 alloc] init];
...
@implementation _SRRunLoopThread2 {
